...
| **AZIM 1P-IM midstream: ~307 executed loads / ~5.05M units WK24–30; net transport avoidance ~$145K; ~$0.85M annualized (vol-weighted)** | MEASURED+EST | `_notes/azim_entitlement_ytd_deepdive_2026-07-21.md` ("RAIL LEGS FIRMED" + "CORRECTION 2026-07-24") + `_loads.tsv` (per-VRID VLS actuals; model = avoided_OTR − (Y_full − X_full), rail priced INSIDE the run structures) + `promo_doc/evidence/clt2_mci4_verification_2026-07-24.md` | 2026-07-24 | **CORRECTION 2026-07-24 (Will):** removed the entire **CLT2→MCI4 lane (24 req / 16 counted)** — it has **NO new manifest ID in the AZIM midstream SharePoint**, which is the entitlement's own definition of "executed," so it never belonged in the base. (Root cause: Will's midstream-AZIM checker / menu-dashboard flagged non-redirectable 1P as redirectable → bad request that was requested but never approved/executed. SIM V2251735064 has only a general "20 FL" approval, no re-approval for the escalated 26-load 1PIM ask; VLS shows same-ramp, no avoided rail leg — NOT crosstown.) The lane was NEGATIVE (−$7,394), so removal RAISES YTD: **~$137K → ~$145K**, annualized **~$0.85M = $145K ÷ 16.7%** (2025 seasonal share) / ~$1.08M flat. Prior FIRMED number was $137K/323 loads/5.39M u (superseded "$180K/$1.1M" and "$267K/$2.5M" earlier). RDU2→TOL3 rail credit unchanged (genuinely crosstown, avoided leg MEASURED ~$996/load; waybill re-rates — Will confirmed 7/24). Cost SOURCE = VLS `ats-onestopshop.v_load_summary`; fallback = AZIM billing / procurement IM lane rates. |
