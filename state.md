...
- **Life main:** Become my own point of origin — self-validation, worth from inside not the scoreboard. Keystone: **Friday 7 AM boxing technique class** (cut phase; BJJ + Thursday Muay Thai resume post-cut Jul 12+). Measure inputs, never score outputs. `goals/desire-polarity.md`.
...
- **Combat keystone reset (6/1):** BJJ deferred to post-cut (Jul 12+) — high fatigue-per-calorie pulls recovery from the lifts on a deficit. Cut-phase keystone is now **1× Friday boxing technique class (no sparring)** — embodied/presence work at a fraction of the recovery tax. Book trial this week. Style/grooming overhaul sprinting in parallel (next 2–3 weeks).
...
- **Solo-pursuit keystone (cut phase, reset 6/1): Friday 7 AM boxing technique class (no sparring), 1×/wk.** Replaces BJJ for cut sustainability — same slot, same role in the Life quest. Book trial this week. BJJ + Thursday Muay Thai both ramp post-Jul 11.
...
- **Dan Nguyen partnership thesis:** Scenario B resolves the original $500k–$1M debt-load objection that put it dormant May 2025. **Reactivation = next-week move.** Reach-out frame B (direct ask). AZ CPA 51% paths kept open: retain CPAs as equity ≥51% OR dodge CPA-only services (decided when concrete deal forces it). Roland Frazier value/opportunity outreach script v2 + Apify Google Maps scrape + Claude Code lead-enrichment owed before message goes. Prep work Mon–Thu, message readiness-gated. Dan thesis history captured `journals/dan-thesis-history.md` + `context/dan-thesis.md` (workflow partial — verify per task #73).
