...
- **Location criteria (7/12, final set):** sunny + moderate (no AZ extremes/dryness, not gray-rainy) · **strong bachata sensual scene** (quest-infrastructure — dance is a standing desire-polarity move) · **closer to Northwest Arkansas where possible**. NWA test = **direct flights viable, not just drive distance** (refined 7/12); KC ELIMINATED (bachata scene too weak, Will's read). **Dallas leads** (top-tier scene, 0% tax, ops market, ~2h PHX, 1h DFW→XNA); **Atlanta = real #2** (strong scene, logistics-capital job market, mild winters, direct ATL→XNA); Houston = scene ✓ but swamp-summer climate test; NYC = scene-maximalist outlier. 2027 scouting: Dallas + Atlanta first.
